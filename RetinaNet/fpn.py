...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        if stride != 1 or in_planes != planes * self.expansion:
            self.downsample = nn.Sequential(
                nn.Conv2d(in_planes, planes * self.expansion, kernel_size=3, padding=1, stride=stride, bias=False),
...
class FPN(nn.Module):
    def __init__(self, block, num_blocks):
        ...
        # Lateral layers
        self.lateral_layer1 = nn.Conv2d(2048, 256, kernel_size=1, stride=1, padding=0)
        self.lateral_layer2 = nn.Conv2d(1024, 256, kernel_size=1, stride=1, padding=0)
        self.lateral_layer3 = nn.Conv2d(512, 256, kernel_size=1, stride=1, padding=0)

        # Top-down layers
        self.top_layers1 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
        self.top_layers2 = nn.Conv2d(256, 256, kernel_size=3, stride=1, padding=1)
